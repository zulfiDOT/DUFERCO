...
export default class Lookup extends LightningElement {
    ...
    setSearchResults(results) {
        ...
        //const cleanSearchTerm = this.searchTerm.replace(REGEX_SOSL_RESERVED, '.?').replace(REGEX_EXTRA_TRAP, '\\$1');
        const regex = new RegExp(`(${this.searchTerm})`, 'gi');
        this.searchResults = results.map((res) => {
            if (this.searchTerm.length > 0) {
                //res.title = res.title ? res.title.replace(regex,'<strong>$1</strong>') : res.title;
                res.formattedTitle = res.title ? res.title.replace(regex,'<strong>$1</strong>') : res.title;
            }
