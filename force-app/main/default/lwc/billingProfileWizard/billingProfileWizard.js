...
export default class BillingProfileWizard extends LightningElement {
    ...
    handleSelectedPayment(event){
        this.paymentID = event.detail.Id ? event.detail.Id : event.detail;
        this.handleNext();
    }

    handleSelectedAddress(event){   
        this.addressID = event.detail.Id ? event.detail.Id : event.detail;
        this.addressToClone = new Object();
        if(event.detail.Type__c && event.detail.Type__c != 'Indirizzo Fatturazione'){
            this.addressToClone =  Object.assign({}, event.detail);
    ...
    saveStep1(){
        this.template.querySelector('c-step-payment-method').handleNext();
    }

    saveStep2(){
        this.template.querySelector('c-step-shipping-address').handleNext();
    }

    saveStep3(){
        this.template.querySelector('c-step-billing-profile').handleBillingSubmit();
    }
